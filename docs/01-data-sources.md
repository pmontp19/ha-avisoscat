...
La recerca es va fer al repositori d'[`ha-incendiscat`](https://github.com/pmontp19/ha-incendiscat),
amb qui aquesta integració comparteix convencions d'enginyeria, i es va traslladar aquí en
crear el repositori propi.
...
| 10 | `comentari`, `llindar*`, `meteor.nom` són **text extern no fiable** | Mai `allow_html`, mai interpolació HTML directa (regla de `CLAUDE.md`) |
| 12 | L'avís de **temps violent** porta `afectacions` **penjant directament de l'avís** (`avis["afectacions"]`), sense `evolucions` ni `periodes` | `Avis` té el camp `afectacions_directes` i `_parse_avis()` el llegeix a més de `evolucions`. **Els consumidors han de llegir l'agregador `Avis.totes_afectacions`**, mai un dels dos camps sol: `afectacions_directes` és buit en un avís ordinari i `Evolucio.afectacions` és buit en un de temps violent, de manera que llegir-ne només un torna a donar «cap perill» en silenci. `Avis.perill_maxim` llegeix l'agregador, així que un `Avís Vigilància per Temps Violent` de **grau 6 vigent** ja llegeix `perill_maxim == 6`, no 0. Cobert per `test_trap_12_violent_weather_afectacions_hang_directly_off_the_avis` a `tests/test_models.py`. Tractar-ho com una franja única de 2 h des de `dataEmisio` és feina de `vigencia.py` (§5), ja cobert per `_projecta_temps_violent` amb `FINESTRA_TEMPS_VIOLENT` |
| 13 | Els `dia` poden ser **tots il·legibles alhora** (un canvi de format de data amunt els rebutja de cop) i llavors no hi ha data per situar cap franja | Derivar el dia per posició: `dataInici.date()` + índex de l'evolució dins l'avís, perquè la captura del 2026-08-05 porta les evolucions en ordre cronològic diari començant justament a la data del `dataInici` (2026-08-04, 2026-08-05, 2026-08-06 amb `dataInici` 2026-08-04T12:00). És una **inferència, no un fet**: `vigencia.py` la comprova contra el `dataFi` de l'avís i l'horitzó de 3 dies, i si no s'aguanta ho reporta un cop per emissió (warning el primer, debug després, amb el meteor identificat) i col·lapsa tots els dies al `dataInici` deixant **una sola projecció per (dia, franja), la més greu**, amb desempat determinista i mai per ordre d'arribada (§5 de [`04-architecture.md`](04-architecture.md)) |
...
§9 imposa per als FeatureServers ArcGIS: **`.get()` amb valor per defecte, mai indexació
directa**.
